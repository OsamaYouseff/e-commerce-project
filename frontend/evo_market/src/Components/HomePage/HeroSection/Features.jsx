...
const Features = () => {
    return (
        <Stack direction={"row"} alignItems={"center"} justifyContent={"space-center"} gap={1} marginTop={".9375rem"}
            ...
            sx={{ bgcolor: "categoryColor.main", borderRadius: ".375rem", px: "1rem", py: ".5rem", boxShadow: 3, }}
        >
...
function BoxFeature({ icon, title, subtitle }) {
    ...
    return (
        <Stack className="border" direction={"row"} alignItems={"center"} justifyContent={"center"} gap={2} sx={{
            ...
            "&:hover": { border: `.0625rem solid ${"text.primary"}` }, transition: "all 0.35s ease", width: "12.5rem",
            ...
            <Box >
                ...
                <Typography fontSize={".8125rem"} color={"text.secondary"}>{subtitle}</Typography>
            </Box >
        </Stack >
    )
}
